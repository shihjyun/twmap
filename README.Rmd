...
`twmap` 是純資料集套件，蒐集了國土測繪中心(NLSC)所繪製的縣市、鄉鎮市區及村里界圖，並以 `sf` 套件的 [simple feature](https://r-spatial.github.io/sf/articles/sf1.html) 型別儲存，因此如果要對資料進行操作的話，建議與 `sf` 套件搭配使用👍。
...
- 更改投影方式 :
  ...
  可以直接使用 `sf` 套件的 [`st_transform()`](https://r-spatial.github.io/sf/reference/st_transform.html) 函式進行轉換！

- 讓地圖檔案變小：

  使用 [`rmapshaper`](https://cran.r-project.org/web/packages/rmapshaper/vignettes/rmapshaper.html) 幫地圖瘦身！
  
- 輸出成各種不同的檔案類型：

  [`st_write()`](https://r-spatial.github.io/sf/reference/st_write.html) 可以將地圖物件輸出成一些常見的檔案類型(`.shp`, `.geojson`   etc.)，而如果要輸出成 `.topojson` 的話可以使用 [`geojsonio`](https://github.com/ropensci/geojsonio)
